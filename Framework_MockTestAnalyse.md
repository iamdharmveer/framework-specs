...
#   input (Q9) — the content signature is derived from the questions themselves and maps
#   aptitude-class corpora to the five legacy masking modes (byte-identical output, EC-26)
#   while content-class corpora (science/engineering/medical/law/commerce, previously
#   silently mislabelled 'reasoning') get v2 masking that keeps notation and units.
#   ADDED: 24-mechanic ordered classifier (unknown REPORTED never defaulted), 15-value
#   option-shape vocabulary, Axis-2 ladder 8→11 (IDENTIFY / SELECT_PLOT / RANK; STATEMENT
#   widened per E-7), pattern_keys (KEY = mechanic|form|polarity|answer_type|option_shape;
#   S7-STYLE composes from these, never from template text — RC-3), numeric distractor
#   mining, per-unit number ranges {min,max,p10,p50,p90,n} + PYQ_EXCLUDE_VALUES (EC-49
#   universal-constant exemption), EC-9 low-entropy detection, stimulus_stats (Q11), and
#   medium stamping (§6.1.9: style computed on 'en' renderings; other-medium counted).
#   THREE-ARTEFACT CONTRACT (§6.1.8): the SAME PYQExtract run now also writes
#   <EXAM>_style_profile.json (activation-gated, DORMANT is a written status with a
#   reason, never a missing file — P-3) and <EXAM>_pyq_index.json (shingle/hash index,
#   no question text), both stamped with the SAME corpus_hash (sha256 over sorted
#   per-paper file hashes, stamped by process_pyq_paper). E-13: stem/option text now
#   reads corpus_io.text_of(para) so <w:t>/<m:t> interleave in document order;
#   enrich_paragraph_with_omml retained for flags only. E-3: content-path approach
#   derives from the measured mechanic (MECHANIC_TO_APPROACH); the aptitude path keeps
#   infer_approach VERBATIM. AXIS2_CLASSES defined ONCE in blueprint_core (P-7).
#   Additive-only: every pre-existing section_rules field is byte-identical on the
#   aptitude reference corpus except the version stamps. Engine self-test 120 → 183
#   (+50 style fixtures, +13 mutation-kill pack; B4 pipeline mutation within budget 1).
# v2.55 — 2026-08-27 — GAP-2026-08-27-DIFFICULTY-PROFILE. E-9 keyword difficulty scorer
#   RETIRED (blueprint_core.score_difficulty, analyse_engine adapter, build_diff_criteria
#   deleted; Step 5 writes no PYQ_DIFFICULTY_CALIBRATION; QV-3 reports 'retired'). Difficulty
#   is measured once, by PYQExplain §7A with the derivation rubric, into the exam's
#   difficulty profile (PYQExplain S7A-6 / blueprint_core Cluster DP), which Blueprint §S7-0
#   and Step 7 / ScopedBlueprint read. EC-6 marks no longer scale a threshold. §14 schema and
#   MockCreate handoff text updated. analyse_engine self-test 120 (−3 E-9, +2 guards).
# v2.54.1 — 2026-08-23 — GAP-2026-08-23-STEP5-YEARLESS-TEMPLATES (P1) + DOC SYNC.
#   PATCH bump: emitted stamps stay v2.54; the golden (all-dated) corpus is
#   byte-identical — the fix is a filter that is a no-op when every year is set.
#   THE DEFECT. generate_templates (analyse_engine) read
#       years = sorted(set(q.get('year', 2020) for q in questions))
#   Extraction stamps q['year'] UNCONDITIONALLY (process_pyq_paper), so a paper
#   whose filename carries no year stores None WITH THE KEY PRESENT — and a .get
#   default never fires for a present key. sorted() over {int, None} raises
#   TypeError inside synthesise_subtopic, which runs for every subtopic: a hard
#   stop of the WHOLE synthesis for any corpus that mixes dated and undated paper
#   filenames. Found by executing this file's own fences end-to-end against a
#   production-shaped corpus (the D1/D2 method); every static gate was green.
#   SAME FAMILY as D7 and ONE FUNCTION from D7's own fix — which survived because
#   the D7-era fixtures modelled a year-less question as key-ABSENT, the one shape
#   the dead default could satisfy. A fixture in the wrong shape consumes the
#   suspicion that would have found the defect (the D7 lesson, replayed).
#   SECOND SITE swept in the same function: the cluster 'years' set collected
#   i['year'] unfiltered, so the identical crash waited one line behind the first.
#   FIX. Year-less questions drop out of the RECENCY basis only — years/last2 and
#   per-pattern years now filter None — exactly the rule subtopic_option_format
#   already applies to its recency vote (D7). All-dated corpora are unchanged. An
#   all-undated corpus previously emitted years=[None] and stamped observed_recent
#   from a None-in-{None} membership accident; it now emits years=[] with
#   confidence from raw_count, which is what the D7 principle requires. Engine
#   fixtures added in the PRODUCTION shape (year key present, value None): mixed,
#   all-None, and an all-dated equivalence guard.
#   DOC SYNC in the same release (no behavioural rule changes beyond the above):
#   * PARAMETER 4 still described the pre-v2.50.0 auto-redirect to --synthesise
#     ALL on an empty corpus — the exact fall-through EC-P39 forbids and this
#     file's own DoD [25] names as the dangerous path. Rewritten to state EC-P39.
#   * §1-6 / compute_coverage_mode carried contradictory inserted lines (a
#     "guideline, not a blocker" sentence inside a NON-NEGOTIABLE rule, and a
#     phantom min_pyq_years project/trigger parameter that no parser anywhere
#     reads). Aligned to the enforcement code: min(5, available), never waivable,
#     with fewer-than-5-years exams processing ALL available years.
#   * E-9/E-10 CROSS-FILE SYNC RULE still demanded a byte-identical mirrored COPY
#     in blueprint_core; v2.30 replaced the copy with a delegating adapter.
#     Restated as a delegation rule so no editor re-introduces a second copy.
#   * DoD [4] said "QV-1 through QV-12"; the suite is QV-1..QV-16 (+QV-5b,
#     +QV-13b). S11-2 PART A's example extended to show all 16.
#   * DoD numbering collision: the v2.24.6 additions in §15 reused [24]-[26],
#     numbers already taken by the transport/corpus items in the main DoD list.
#     Renumbered [27]-[29]; content unchanged.
#   * S8-7 schema now documents q_num / question_type / terminated_by_heading
#     (stamped at the one emission point since v2.37/v2.39) and that year is
#     null when the paper filename carries no detectable year.
#   * S8-0b's incident byte/line counts annotated as historical measurements —
#     the live numbers come from bootstrap and SPEC_SECTIONS.json at run time.
# v2.54.0 — 2026-08-23 — STAMP-DEDUP + TRANSPORT-HELPER EXTRACTION (Items 3a+3b;
#   MINOR — changes one artefact line per section_rules.md). 3a: the section_rules
#   header line read "# Generated by Generated by ..." since v2.3 —
#   GENERATED_BY_STAMP already begins with "Generated by" and write_section_rules
#   prepended it again (analyse_engine L2900). Fixed at the f-string; stamps
#   cascade v2.53 -> v2.54 (MS-3). EXPECTED ARTEFACT DIFF, documented before
#   capture: the dedup line + every v2.53 stamp line; nothing else. Golden set
#   RE-CAPTURED from this release's output — prior hashes are superseded and
#   recorded in the harness doc as the pre-fix reference. 3b: the six transport
#   helpers left this file's fences for transport_core.py — build_session_re,
#   make_drive_list_fn, make_paper_id, is_already_processed VERBATIM;
#   sort_papers_recency_first gains a session_re PARAMETER (it closed over the
#   session global SESSION_RE — an engine cannot); collect_pyq_papers gains a
#   required listing_cache parameter. Both keep one-line DELEGATING ADAPTERS here
#   preserving the session defaults (SESSION_RE, DRIVE_LISTING_CACHE), per the
#   DELEGATION contract. collect_pyq_papers.last_rejects attribute DROPPED —
#   read nowhere in the corpus (verified by grep); rejects remain ATTACHED via
#   the returned tuple (the v2.29 rule, unchanged).
...
#   ARTEFACT VALUES CHANGE — all six IIT_JAM_MATHEMATICS artefacts verified
#   byte-identical against the golden set from deployed 2026.08.19.3.
#
#   WHAT MOVED. One fence, 346 lines, 8 functions, ZERO top-level session-flow
#   statements: acquire_listing, probe_transport, probe_drive_channel, plan_transport,
#   acquire_paper, read_transport_verdict, record_transport, log_session, plus the
#   derived constant SESSION_INLINE_BUDGET. The same definition/flow split that governed
#   batches 2, 3 and 5, applied unchanged. Spec python 2,352 -> 2,051 lines.
#
#   WHAT DID NOT MOVE, AND WHY THE BLOCK WAS EXTRACTABLE AT ALL. gdrive_search and
#   gdrive_download_file stay here. They are CLASS T markers — operations the MODEL
#   performs in its own turn — and an engine cannot perform a tool call. Every function
#   that moved takes the RESULT of a CLASS T operation as an argument (pages,
#   probe_payload, drive_payloads, resolver), which is exactly what made the boundary
#   clean.
#
#   WHY NOW. This fence was INVISIBLE until release 2026.08.20.2. It carries a triple
#   backtick inside a docstring, and four auditors extracted fenced python with a
#   non-greedy regex that ended at the first such marker — so the fence was cut
#   mid-string, failed to parse, and was discarded by each caller's parse guard. Nothing
#   failed; the tools simply never read the Drive transport contract, and code that is
#   never read produces no findings. The line scanner made it readable. This release
#   makes it TESTABLE: 49 self-test fixtures and a mutation gate, neither of which can
#   exist for code that lives in a fence.
#
#   VERIFICATION IS NOT THE GOLDEN SET, AND SAYING SO MATTERS. run_synthesise — the
#   path `--synthesise ALL` exercises — calls NO function in this block, so the golden
#   set proves the synthesis is unchanged and proves nothing about the transport. What
#   stands in its place: the extraction is byte-verbatim (the divergence baseline's
#   fingerprints for plan_transport, acquire_paper and probe_drive_channel are IDENTICAL
#   before and after the move, which is the refactor proving itself), 49 fixtures that
#   inject every dependency rather than touching a filesystem, and a pipeline mutation
#   gate at 3/3.
#
#   THE STEP 4 DIVERGENCE SURVIVED THE MOVE — BY ONE LINE OF FORESIGHT AND ONE
#   CORRECTION. Framework_PYQCount S5-0 defines its own probe_drive_channel,
#   plan_transport and acquire_paper; the divergence is declared and pinned in
#   XSPEC_DIVERGENCE_BASELINE.json. When this batch landed, all three entries reported
#   "no longer diverges" and demanded deletion — the ratchet was right that something had
#   changed and wrong about what. Deleting them would have let this refactor quietly
#   retire the only check watching those pairs. audit_deep now reads ENGINE definitions
#   as well as spec fences, and a new XSPEC-ENGINE-DRIFT check reports a spec that
#   re-localises an engine-defined function without delegating, so the exemptions
#   suppress a real finding again instead of being decorative.
#
# v2.53.4 — 2026-08-20 — WAVE 2 PART C, BATCH 7. DEFECT D7, AND THE WRITERS ENTER THE
#   MUTATION GATE. PATCH bump: the emitted stamps stay at v2.53 and NO ARTEFACT VALUES
#   CHANGE — all six IIT_JAM_MATHEMATICS artefacts verified byte-identical against the
#   golden set from deployed 2026.08.17.1.
#
#   D7 (P1) — ONE QUESTION WITH NO YEAR CRASHED THE WHOLE SYNTHESIS.
#   subtopic_option_format() bucketed formats with `by_year.setdefault(q.get('year',
#   '?'), ...)`, then sorted those keys on the very next line. The '?' sentinel puts a
#   str into a dict whose other keys are ints, so the moment a corpus contained BOTH a
#   year-bearing question and a year-less one:
#       TypeError: '<' not supported between instances of 'str' and 'int'
#   raised inside synthesise_subtopic(), which runs for EVERY subtopic. Same family as
#   D1/D2/D6: a hard stop of Step 5, invisible on the reference corpus because every
#   IIT_JAM_MATHEMATICS question happens to carry a year. Across ~200 exams a single
#   failed year extraction is not an edge case, it is a Tuesday.
#
#   The same sentinel had a second effect: it made the function's own `if not years:`
#   guard UNREACHABLE. by_year gained a key for every question, so `years` was never
#   empty and the branch written for a year-less corpus could never run. Year-less
#   questions are now dropped from the RECENCY vote only — they cannot inform which
#   option format is most recent — which fixes the crash and restores that guard to the
#   job it was written for. Equivalence is exact where it matters: with every question
#   carrying a year (the golden set) by_year is unchanged; with NO question carrying one
#   the old path returned recent_format == primary and changed_recently == False, which
#   is precisely what the restored guard returns.
#
#   HOW D7 WAS FOUND, which matters more than D7. The mutation gate reported the
#   `sorted(` on that unreachable branch as a SURVIVING MUTANT while a fixture named
#   `no_year_branch_all_observed_sorted` — added one release earlier — passed. Both
#   statements were true at once, and only one reading explains both: the fixture never
#   reached the line it was named after. A fixture that passes without executing the
#   code it claims to cover is worse than no fixture, because it also consumes the
#   suspicion that would have found the defect.
#
#   THE WRITERS ARE NOW TESTED AT ALL. Nine of the twenty surviving mutants lived in
#   five functions the engine self-test had NEVER CALLED — synthesise_subtopic,
#   compute_section_axis_distribution, write_section_rules, write_subtopic_manifest and
#   rebuild_subtopic_manifest_from_section_rules. The reason was mechanical, not
#   editorial: each writes to a hardcoded '/mnt/user-data/outputs/', which does not
#   exist on a CI runner, so no fixture COULD call them. An additive `out_dir=None`
#   (default unchanged, every production call site untouched) removes that blocker —
#   the same remedy `search_dirs` applied to load_mechanic_overrides after
#   GAP-2026-08-17-B4-ENV-SKEW. An `if os.path.isdir(...)` guard around the fixture
#   would NOT do: a guarded assertion is a dormant one, and a dormant assertion is
#   exactly where a survivor hides while CI reports green.
#
#   The new fixture is a different SHAPE from B5/B6, which asserted on directly-callable
#   pure functions. The remaining ordering defects live BEHIND the writers, so a small
#   corpus is driven end to end — questions -> synthesise_subtopic -> write_section_rules
#   -> write_subtopic_manifest -> rebuild — and the assertions read the artefacts back.
#   It is adversarial by construction, not merely representative: four sections colliding
#   on one prefix supplied out of alphabetical order (1 in 24), four papers whose figural
#   counts are all different so the per-paper series is a permutation detector rather
#   than a bag of equal numbers (1 in 24), and four years whose set-iteration order is
#   not their sorted order. A fixture whose values are all equal cannot see an ordering
#   defect at all; that is how nine of these survived a suite of eighty-six checks.
#
#   QV-13 WAS HALF A GATE. It is named MECHANIC IDENTITY INTEGRITY and identity is the
#   PAIR (collision_domain, form_key) — the uniqueness test buckets by exactly that
#   pair. It re-derived form_key to detect drift and threw the freshly-derived
#   collision_domain away, so a drifted domain passed unnoticed while the check that
#   exists to catch drift reported PASS. Not hypothetical: build_section_prefix_map
#   assigns colliding prefixes by iteration order, which is the defect v2.53.3's B6
#   batch fixed in mint_subtopic_ids and stamp_mechanic_axes. Those two were pinned; the
#   GATE that notices when they drift was not. QV-13 now compares both halves, with the
#   same curator-override escape hatch form_key already had.
#
#   DIAGNOSTIC TEXT IS PART OF THE CONTRACT. Five survivors were a `sorted(...)` inside
#   a hard-stop message, and the tempting verdict is "equivalent mutant — only word
#   order in an error changes". It is not. Each enumerates a SET, so the same corrupt
#   overrides file produces a differently-worded refusal on every run: a curator cannot
#   diff two runs, cannot grep a log for a known message, and cannot tell a second fault
#   from the same fault re-ordered. Artefact nondeterminism (D4) applies to what the
#   operator READS, not only to what the writers EMIT. Each message is now asserted
#   verbatim, with three or more members, because a two-element set is a coin flip.
#
#   MEASURED. Pipeline mutation survivors 20 -> 1; score 60.0% -> 97.9%; engine
#   self-test 86 -> 116 checks. The single remaining survivor is PROVEN EQUIVALENT and
#   documented at its own site: `years_seen` in _compute_structural_changes is read on
#   four lines and every one is order-invariant (truthiness, max, min, len), so no
#   fixture can kill it and none is faked. MUTATION_BUDGETS.json ratchets 20 -> 1.
#
#   AND THE WATCHER GAINED A WATCHER. audit_mutation.py — the gate whose entire job is
#   proving other tests are connected — had no --self-test, and validate.yml never ran
#   one. The cost showed up here: a `sorted(` inside a NESTED test helper's DOCSTRING was
#   listed as a live target and counted as a surviving mutant of the production engine.
#   The scaffolding exclusion tested only the innermost frame, so a helper defined inside
#   self_test was fair game. It now tests the whole enclosing chain, ships ten fixtures
#   plus a meta-assertion, and runs as a blocking CI step before the gate it guards.
#
...
# v2.54.0 (Item 3b): build_session_re moved VERBATIM to transport_core.py.
import transport_core
from transport_core import build_session_re
...
# v2.54.0 (Item 3b): make_drive_list_fn moved VERBATIM to transport_core.py —
# including the flat-cache ROOT-only scoping that prevents DuplicatePaperError
# (measured 2026-08-15). Bound via the transport_core import above.
from transport_core import make_drive_list_fn


def collect_pyq_papers(folder_id, listing_cache=DRIVE_LISTING_CACHE):
    """DELEGATING ADAPTER (v2.54.0, Item 3b) — the body lives in transport_core.py
    where listing_cache has no engine-side default; this adapter supplies the session default
    (DRIVE_LISTING_CACHE). Returns (papers, rejects) in DRIVE LISTING ORDER —
    recency belongs to sort_papers_recency_first (S8-0 / EC-X21); rejects are
    ATTACHED via the tuple (v2.29). The old .last_rejects function attribute is
    DROPPED — read nowhere in the corpus (verified 2026-08-23)."""
...
PARAMETER 4: no-PYQ behaviour (mode decision, not a tunable parameter)
  v2.54.1: a PYQ-less exam is requested EXPLICITLY with 'PYQExtract --synthesise
  ALL'; it is NEVER inferred. If the trigger supplied a PYQ: link and zero usable
  papers came back, that is a TRANSPORT diagnosis and EC-P39 HARD STOPS the run —
  Step 5 does NOT fall through to synthesis. (The pre-v2.50.0 text here described
  an auto-redirect on an empty corpus; that fall-through shipped complete green
  deliverables of zero-PYQ scaffolds for exams with a full Drive folder, and DoD
  [25] exists to forbid it. This paragraph was the last place still teaching it.)
  In explicit --synthesise ALL mode: all subtopics written as confidence='absent';
  Step 7 uses training knowledge. Valid and expected state — not an error.
...
def compute_coverage_mode(pyq_doc_paths):
    ...
    MANDATORY (v2.54.1 — aligned with the §1-6 rule text and the S8-4 enforcement
    code, which have always required this): process the min(5, available) most
    recent years. If fewer than 5 years exist (new exam, limited PYQ history),
    ALL available years must be processed — the requirement then equals exactly
    what exists, so a young exam is never blocked. The rule cannot be waived.
    (There is no min_pyq_years configuration or trigger parameter; an earlier
    revision of this docstring referenced one that no parser anywhere reads.)
    """
...
RULE:
  Analysis MUST cover the min(5, available) most recent years of PYQ papers.
  If fewer than 5 years of papers exist for the exam, ALL available years must be
  processed — the requirement caps at what exists, so a young exam is never
  blocked, and the rule is still never waivable. (v2.54.1: two inserted lines
  here previously called the 5-year rule "a guideline, not a blocker" and cited
  a min_pyq_years parameter that does not exist — contradicting the NON-
  NEGOTIABLE banner above and the S8-4 enforcement code, which block synthesis.)
...
### E-9 — Difficulty scoring — RETIRED (v2.55, GAP-2026-08-27-DIFFICULTY-PROFILE)

  The 3-axis keyword scorer (`score_difficulty`, C/I/V on stem vocabulary) is
  RETIRED from the corpus: `blueprint_core.score_difficulty` and the
  `analyse_engine` adapter are deleted, Step 5 no longer labels PYQs, and the
  `PYQ_DIFFICULTY_CALIBRATION` block is no longer written to section_rules.md.

  WHY. A vocabulary-based scorer cannot be exam-agnostic ("ratio of … two" scored
  a question hard whatever its maths; a three-statement syllogism scored easy) and
  it disagreed with the derivation-observed rubric by 40 points on a real paper
  (SSC CGL Tier-1 09-Sep-2024: 95/5/0 vs 56/44/0). Two scales in one corpus is
  how drift starts.

  WHAT REPLACES IT. Difficulty is measured ONCE, by the step that SOLVES every
  question: PYQExplain §7A records the six derivation observations and the raw
  rubric score (`blueprint_core.difficulty_score`, Cluster E2 — the same function
  MockTestCreate CHECK 3c and MockTestExplain §7A-M enforce) and, on its final
  batch, writes `[ExamCode]_difficulty_profile.json` (PYQExplain S7A-6, engine
  Cluster DP). From that file: Blueprint §S7-0 derives the DEFAULT
  Easy:Medium:Hard split per section; Step 7 / ScopedBlueprint read per-subtopic
  calibration examples (`bc.dp_calibration`). Step 5's remaining difficulty-
  adjacent field is `PYQ_IMAGE_ANALYSIS.complexity_dist` (figural complexity,
  unrelated to the rubric) — unchanged.

  E-9 numbering is retained so cross-references stay resolvable; nothing here is
  executable.

### E-10 — Template generation (subject-aware variable stripping)

> **SINGLE-DEFINITION RULE (v2.54.1; supersedes the v2.24.10 mirror rule):**
> `determine_strip_mode` also has its ONE implementation in `blueprint_core.py`
> Cluster E; the analyse_engine name is a delegating adapter (v2.30). Edit the
> Cluster E body only — same rule as E-9, for the same reason.
...
```python
# IMPLEMENTATION EXTRACTED from §5 (2026-08-17, v2.53.3) to the repo engine file
#   analyse_engine.py   —   AXIS2_CLASSES, STEM_FORMAT_TO_AXIS2, FAMILY_AXIS2_MENU, _TABLE_WORD_RE, _looks_like_table_stimulus, classify_axis1, classify_axis3, _opts_are_combination_labels, _MATCH_PAIR_RE, _MATCH_PAIR_SUB, _MATCH_OPT_RE, _label_family, _opts_are_match_pairs, classify_axis2, tag_axes, _FAMILY_KEYWORDS, resolve_presentation_family_s5, axis2_capability, compute_section_axis_distribution, FIGURAL_IRREDUCIBLE_RATE, synthesise_subtopic, _detect_recycled_stimuli, _absent_entry, build_diff_criteria, infer_approach, extract_number_ranges, extract_context_pool, extract_passage_structure, stamp_medium, medium_of, derive_content_signature, derive_legacy_mode, strip_variables_v2, _protect_emphasis, detect_option_shape, detect_mechanic, _named_entities, _named_entities_ordered, _annotate_patterns_with_style, mine_distractor_mechanisms, extract_number_ranges_v2, extract_context_pool_v2, compute_style_cell, build_pattern_keys, detect_low_entropy, _pctiles, MECHANIC_TO_APPROACH
# Moved VERBATIM: the block that lived here through v2.53.2 is byte-identical to
# the corresponding region of the engine. Hash-tracked in MANIFEST.json and
# bootstrap-verified. THIS SECTION RETAINS THE CONTRACT; THE ENGINE HOLDS THE CODE.
# The engine is IMPORTED AND EXECUTED, NEVER READ — not a .md, so SKILL Rule 2
...
from analyse_engine import (
    ...
    write_style_profile, write_pyq_index,   # v2.56 §6.1.8 three-artefact contract
)
```
...
EC-6:  MARKS VARIATION (multi-mark exams — e.g., 1-mark and 2-mark questions)
  ...
  v2.55: marks no longer scale any difficulty threshold (E-9 retired); marks_per_q is
  still read for EXAM_STRUCTURE and Step 6 marking. Exam-agnostic; no exam names hardcoded.
...
```python
# IMPLEMENTATION EXTRACTED from §S8-0 (2026-08-20, v2.53.5) to the repo engine file
# transport_core.py, which is IMPORTED AND EXECUTED, NEVER READ. It is not routed by
# SKILL Rule 2 and costs this session nothing.
#
# WHY IT MOVED. This fence carries a triple backtick inside a docstring. Until release
# 2026.08.20.2 four auditors extracted fenced python with a non-greedy regex that ended
# at the FIRST fence marker in the body, so it was cut mid-string, failed to parse, and
# was discarded by each caller's parse guard. Nothing failed — the tools never read the
# Drive transport contract at all, and code that is never read produces no findings.
# The line scanner made it visible; this move makes it TESTABLE: 48 self-test fixtures
# and a pipeline mutation budget, neither of which can exist for code in a fence.
#
# WHAT DID NOT MOVE. gdrive_search and gdrive_download_file stay in this spec above.
# They are CLASS T markers — operations the MODEL performs in its own turn — and an
# engine cannot perform a tool call. Every function below takes the RESULT of a CLASS T
# operation as an argument, which is exactly why they were extractable.
#
# THE STEP 4 DIVERGENCE SURVIVES THE MOVE. Framework_PYQCount S5-0 defines its own
# probe_drive_channel, plan_transport and acquire_paper; the divergence is intentional
# and is pinned, with a reason and a fingerprint for both sides, in
# XSPEC_DIVERGENCE_BASELINE.json. audit_deep reads engine definitions as well as spec
# fences, so extracting this copy does not retire the check that watches those pairs.
from transport_core import (
    SESSION_INLINE_BUDGET,   # bc.INLINE_BUDGET_CHARS // 2 — P4c/EC-P36 double charge
    acquire_listing,         # A1 — persist DRIVE_LISTING_CACHE and ASSERT it (EC-P41)
    probe_transport,         # A0 -> A2 — direct lane first, then the connector (EC-P43)
    probe_drive_channel,     # A2 — classify from ONE real download; PROVE it decodes
    plan_transport,          # partition BEFORE fetching; P4a/P4d/P4e/P4f/P4g
    acquire_paper,           # S8-1 batch-loop acquisition; failure -> upload lane
    read_transport_verdict,  # EC-X5/EC-X7 — reuse a recorded channel, never re-probe
    record_transport,        # P4b — persist the verdict; papers_planned is a FORECAST
    log_session,             # G-8 — what this session ACTUALLY did
)
# CONTRACT RETAINED HERE so the reader sees the shape without opening the engine:
#   plan_transport(pending_recency_sorted, channel, session_budget, batch_size,
#                  probe_consumed=0) -> partition dict
#       PRECONDITION: the input is ALREADY recency-sorted (P4d / EC-X21).
#       bc.partition_by_transport admits in the order it receives, so listing order
#       silently admits the wrong years while papers arrive successfully.
#       probe_consumed is NOT optional in practice — audit_callgraph C10 fails the
#       build for a partition call preceded by a CLASS T acquisition without it.
#   acquire_listing(pages, cache_path, folder_id, observed_count) HARD STOPS on a short
#       listing (ListingIntegrityError). Never a TransportFallback: there is no other
#       lane for a corpus that cannot be enumerated. EC-P39 caught zero; nothing caught
#       21-of-22, and 21-of-22 is the dangerous one.
```
...
any work. For trigger PYQExtract that was, AT THE 2026-08-16 INCIDENT, this file (then
8,850 lines / 504,240 B) plus Framework_DeliveryFooter.md (then 929 / 52,594):
556,834 B, ~139,208 tokens, >=36 view calls, MANDATORY, every session. (Sizes
here are the incident's measurements; the LIVE numbers are printed by
bootstrap --trigger and generated into SPEC_SECTIONS.json — v2.54.1.) In the reference incident 40 of session 1's 50 tool
...
would leave session 1 of every one of the ~200 exams exactly as broken as the incident.
  ...
  NON-FINAL  papers_remaining > BATCH_SIZE
             ...
             Measured read set at the incident: 232,228 B, ~58,000 tok — 47% of
             the file then. (Live reduced-set figures: SPEC_SECTIONS.json.)
...
def sort_papers_recency_first(paper_list):
    """DELEGATING ADAPTER (v2.54.0, Item 3b) — the body (year/date/session sort,
    dateless-last rule, MONTH_MAP) lives in transport_core.py, which takes the
    compiled session regex as a PARAMETER: the old body closed over the session
    global SESSION_RE (built from exam_config.json's keyword), and an engine
    cannot hold a session closure. This adapter supplies it."""
    return transport_core.sort_papers_recency_first(paper_list, SESSION_RE)
...
# v2.54.0 (Item 3b): make_paper_id moved VERBATIM to transport_core.py.
from transport_core import make_paper_id
```
...
def run_synthesise(exam_code, progress, coverage_mode='mandatory_5yr',
    ...
    # ── v2.56 (GAP-2026-08-29-STYLE-FIDELITY §6.1.8) — THREE-ARTEFACT CONTRACT ──
    # The SAME synthesis run writes the style profile and the PYQ index, stamped
    # with the SAME corpus_hash as each other. One run, one corpus, one hash:
    # Step 7's G-STYLE/G-PYQ-DIST refuse artefact pairs whose hashes disagree, so
    # the profile can never describe a different corpus than the index.
    # Hash source: _meta['paper_file_hashes'] (stamped by process_pyq_paper from
    # the docx bytes). A pre-v2.56 progress file lacks the key — the fallback
    # hashes paper_ids instead: DEGRADED but deterministic and corpus-stable,
    # printed so the operator can see which source stamped this run.
    import hashlib, json
    _meta_h   = progress.get('_meta', {})
    _fh_map   = _meta_h.get('paper_file_hashes') or {}
    _pids     = list(_meta_h.get('papers_processed', []))
    _hashes   = [_fh_map.get(p) or hashlib.sha256(str(p).encode()).hexdigest()
                 for p in _pids]
    if _fh_map:
        print(f"corpus_hash source: file bytes ({len(_fh_map)}/{len(_pids)} papers)")
    else:
        print("corpus_hash source: paper_ids (DEGRADED — pre-v2.56 progress file)")
    _q_by_sub = {k: v for k, v in progress.items() if isinstance(k, tuple)}
    profile_path, _profile = write_style_profile(
        exam_code, all_entries, _q_by_sub, _pids, _hashes, exam_meta=exam_meta)
    _all_qs   = [q for qs in _q_by_sub.values() for q in qs]
    index_path, _index = write_pyq_index(exam_code, _all_qs, _hashes)
    _profile['self_repeat_rate'] = _index['_meta']['self_repeat_rate']
    with open(profile_path, 'w', encoding='utf-8') as _pf:
        json.dump(_profile, _pf, ensure_ascii=False, indent=1, sort_keys=True)
    print(f"style_profile: {_profile['activation']['status']}"
          + (f" ({_profile['activation']['dormant_reason']})"
             if _profile['activation']['dormant_reason'] else '')
...
def deliver_final(exam_code, rules_path, summary_path, qv_results, progress,
    ...
    # v2.56 §6.1.8: MANDATORY style artefacts, written by the same run and stamped
    # with the same corpus_hash. The registry-handoff rule applies to the whole
...
# v2.54.0 (Item 3b): is_already_processed moved VERBATIM to transport_core.py.
from transport_core import is_already_processed
...
```json
{
  ...
  "('[Section Name]', '[Topic]', '[Subtopic]')": [
    {
      "num":1, "q_num":1, "stem":"clean stem text", "stem_raw":"original stem with NOTE",
      "options":["option1","option2","option3","option4"],
      "section":"[Section Name]",
      "topic":"[Topic]", "subtopic":"[Subtopic]",
      "year":2024, "shift":"S1", "paper_id":"[ExamCode]_2024_Shift1",
      // year is null when the paper filename carries no detectable year
      // (coverage_mode 'no_year_info' / mixed corpora). v2.54.1: recency logic
      // filters null; it never crashes on it and never invents a year.
      "has_note":true, "note_text":"(NOTE: Operations on whole numbers only...)",
      "blank_pos":"none", "is_negative":false, "is_msq":false,
      "question_type":"MCQ", "terminated_by_heading":false,
...
PART A — QV results in chat:
  "=== Quality Verification ===
   ...
   v QV-12 Dedup           : PASS
   v QV-13 Mechanic identity: PASS (pre-write HARD STOP on FAIL; QV-13b likewise)
   ! QV-14 Vision coverage  : [PASS | FAIL — footer renders F1 amber]
   v QV-15 Heading-truncated: PASS
   v QV-16 original_q_num   : PASS
   ==========================="
...
taxonomy → progress → summary):
  ...
  MANDATORY — every run:
    ...
    4. [ExamCode]_style_profile.json      <- v2.56: download → upload to [ExamCode] project
                                             (DORMANT is a written status with a reason,
                                              never a missing file — P-3)
...
Per question generated (Step 7 MockCreate, section 17, S17-3):
  ...
  STEP 2 -- difficulty calibration (v2.55):
    section_rules carries NO difficulty block. Step 7 reads the slot's band from
    the Blueprint difficulty_schedule and its calibration examples from the
    difficulty profile (bc.dp_calibration) — see MockTestCreate S7 CHECK 3c.
...
### §14-S — v2.56 ADDITIVE STYLE FIELDS (GAP-2026-08-29-STYLE-FIDELITY §6.1.8)

```
Every field below is ADDITIVE (P-9): a reader that ignores them behaves exactly
as a v2.55 reader did. All are measured, never looked up (P-1).

Per PYQ_STEM_PATTERNS pattern (after `years:`):
  pattern_key      mechanic|form|polarity|answer_type|option_shape (the measured
                   abstraction; S7-STYLE composes from THIS, never the template text)
  mechanic / polarity / option_shape   majority values over the cluster's members

Per subtopic entry (STYLE block, before wrong_option_structure):
  legacy_mode            one of the five legacy masking modes for an
                         aptitude-class signature; None for content-class.
                         DERIVED from the signature — the name tables are retired
                         as inputs (Q9)
  content_signature      the 10 measured axes (numeric/notation density, caps
                         convention, proper-noun / blank / statement / polarity /
                         stimulus rates, label scheme, medium share); internal
                         '_'-prefixed keys are filtered out (fixture-pinned)
  mechanic_mix           {mechanic: share} over 'en' renderings, keys sorted
  mechanic_unknown_count unknown is REPORTED, never defaulted (§6.1.9)
  low_entropy            EC-9: >=80% of unique stems pairwise >=0.50 token-Jaccard
                         (digits masked)
  distractor_mix         mined numeric/verbal mechanisms, or 'unavailable'
                         (no answer keys — stated, never guessed)
  stimulus_stats         Q11: rate, kind mix, length percentiles, questions per
                         stimulus, position mix
  PYQ_EXCLUDE_VALUES     exact given values (universal constants exempt, EC-49)
  PYQ_NUMBER_RANGES      v2 shape per unit: {min,max,p10,p50,p90,n}
                         (legacy {min,max,multiples_of} unchanged on the
                         quantitative aptitude path)
  PATTERN_KEYS block     per-KEY aggregation: frequency (recency-weighted x2 for
                         the two newest years), raw_count, confidence
                         (observed >=3 / observed_recent / inferred), deprecated,
                         years (sorted), exemplars (0.90-collapsed, max 5)

Same-run artefacts (§6.1.8 THREE-ARTEFACT CONTRACT):
  <EXAM>_style_profile.json   activation-gated StyleCells (paper / section x
                              answer_type / subtopic), item_rules I-1..I-8 with
                              measured violation rates (suspended >= 0.10),
                              universal_constants, thresholds (dispersion-computed
                              when questions_en >= 300 across >= 4 papers, else
                              defaults), self_repeat_rate. DORMANT is a WRITTEN
                              status with a reason, never a missing file (P-3)
  <EXAM>_pyq_index.json       per-question shingle/hash records (stem_md5,
                              8-token shingles, 4-token when stem < 12 tokens,
                              option md5s, canonical numeric values, semantic
                              tuple, duplicate_of) — NO question text
  Both stamp the SAME corpus_hash: sha256 over sorted per-paper file hashes
  (process_pyq_paper stamps _meta['paper_file_hashes'] from the docx bytes;
  pre-v2.56 progress files fall back to hashed paper_ids, printed as DEGRADED).
```
...
UNIVERSAL IN THIS SPEC (identical every exam):
  ...
  v2.56 style layer: signature axes, 24 mechanics, 15 option shapes, 11 Axis-2
    classes, pattern-key composition, item rules I-1..I-8, artefact schemas —
    all measured from the corpus; no cue contains a subject word (P-1,
    fixture-enforced)
...
CATEGORY B (per subtopic entry — one block per subtopic):
  *** DOC-ALIAS ONLY *** — "CATEGORY B" is this schema reference's conceptual name for
  ...
  (PYQ_DIFFICULTY_CALIBRATION — RETIRED v2.55; see E-9. Not written, not read.)
...
[4] QV-1 through QV-16 (plus QV-5b; QV-13/QV-13b are pre-write HARD STOPS): all
    reported results are PASS or WARN (zero FAIL checks). (v2.54.1: this item
    said QV-12, predating QV-13..QV-16; the S11-2 PART A example agreed with the
    stale count and is extended in the same release.)
...
#   [27] Frequency Excel completeness invariant holds (master_data_completeness_test):
...
#   [28] Excel Format == manifest Format for every subtopic (excel_manifest_format_parity_test)
#        — guaranteed by construction when all_entries is passed (both read entry['format']
#        from the same dict).
#   [29] DI/PASSAGE detection uses the structural _looks_like_table_stimulus() helper
#   (v2.54.1: these three were numbered [24]-[26], colliding with the transport/
#   corpus items [24]/[25] in the main DoD list. Renumbered; content unchanged.)
...
## ANALYSE-SYL — ERA-AWARE MINING & SAMPLE PAPERS (v2.57 —
## GAP-2026-09-01 §4.4/§4.7/R9; ACTIVE mode; companion pin: requires
## Framework_PYQSort >= v1.21.0 for the era/status fields it consumes)

CONTENT mining (proportions, topic weights, difficulty mixes) draws from
new-era questions plus old-era questions whose crosswalk status is mapped
(RETAINED/MOVED/MERGED/SPLIT at their NEW labels); questions carrying
bc.OUT_OF_SYLLABUS are EXCLUDED from content mining but INCLUDED in the
style signature (R9: an exam's voice — stem length, option style, question
mechanics — survives a syllabus change even where its content does not).
SAMPLE papers (corpus_io.sample_paper_census intake, recorded at 2a) feed
the STYLE pass only — never proportions, never counts. Where a section's
new-era evidence is a template vacuum (zero new-era questions for a
structure the pattern requires), the analysis output MARKS the vacuum
explicitly rather than silently borrowing old-era proportions — Release C
